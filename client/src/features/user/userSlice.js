...
export const fetchUserToFollow = createAsyncThunk('fetch/userstofollow', async (token) => {
...
export const postFollowedUser = createAsyncThunk('post/followedUser', async (postData) => {
...
export const getSearchedUsers = createAsyncThunk('get/searchedUsers', async (postData) => {
...
const userSlice = createSlice({
	...
	extraReducers: {
		...
		[postFollowedUser.fulfilled]: (state, action) => {
			if (action.payload.success) {
				state.status = 'success';
				state.usersToFollow = state.usersToFollow.filter((user) => user._id !== action.payload.followId);
			}
		},
